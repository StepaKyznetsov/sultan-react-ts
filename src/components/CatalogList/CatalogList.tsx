...
import { useActions, useTypedSelector, useClickOutside } from "../../hooks";
...
const CatalogList: React.FC = () => {
  ...
  for (let i = 0; i < filterState.categoriesFilter.length; i++) {
    filteredItems = filteredItems.filter(
      (e) => e.categories.indexOf(filterState.categoriesFilter[i]) !== -1
    );
  }
  ...
  const countProducts = filteredItems
    .filter((e) =>
      e.brand.toLowerCase().includes(filterState.query.toLowerCase())
    )
    .filter(
      (e) => e.price >= filterState.minQuery && e.price <= filterState.maxQuery
    )
    .filter(
      (e) =>
        filterState.brandsFilter.indexOf(e.brand) !== -1 ||
        !filterState.brandsFilter.length
    );
  ...
  return (
    <div className={css.container} ref={wrapperRef}>
      <div className={css.content}>
        <Breadcrumbs links={[{ title: "Каталог", link: CATALOG }]} />
        <BackArrow />
        <div className={css.head}>
          <h2>Косметика и гигиена</h2>
          <Sort filterState={filterState} />
        </div>
          ...
          {countProducts.length > limit &&
            PAGES.map((e) => (
              <div
                key={e}
                onClick={() =>
                  choosePage(e, setCatalogPage, page, countProducts, limit)
                }
                className={e === page ? `${css.activePage}` : ""}
              >
                {e}
              </div>
            ))}
