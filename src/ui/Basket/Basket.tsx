...
const Basket: React.FC = () => {
  ...
  return (
    <div className={css.basket}>
      <div className={css.basketIcon} onClick={() => navigate(BASKET)}>
        <img src="/images/menu/basket.png" alt="basket" />
        {order.length ? (
          <span className={css.basketCounter}>{order.length}</span>
        ) : null}
      </div>
      <div className={css.sum}>
        <span>Корзина</span>
        <div className={css.money}>{parseFloat(Number(sum).toFixed(2))} ₸</div>
      </div>
    </div>
  );
};
