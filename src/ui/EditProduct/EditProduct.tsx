...
const EditProduct: React.FC<IEditProduct> = ({ usage }) => {
  ...
  const confirm = () => {
    ...
    if (usage === 0) {
      if (items.find((e) => e.barcode === +getElement("barcodeInput")))
      ...
      if (
        currentProduct.barcode !== +getElement("barcodeInput") &&
        items.filter((e) => e.barcode === +getElement("barcodeInput"))
          .length === 1
      )
        return notifyExistBarcode();
      changeProduct({
  ...
  return (
    <div className={css.options}>
      ...
      <div>
        <span>Описание:</span>
        <textarea value={currentDescription} onChange={editDescription} />
      </div>
      <div>
        Тип размера:
        <select onChange={setType} value={currentType}>
      ...
      />
      <button className={css.confirm} onClick={confirm}>
        Сохранить изменения
      </button>
    </div>
  );
